...
>
- 5 **newWorkStealingPool(int parallelism)**，这是一个经常被人忽略的线程池，Java 8 才加入这个创建方法，其内部会构建]ForkJoinPool](https://docs.oracle.com/javase/9/docs/api/java/util/concurrent/ForkJoinPool.html)，利用[Work-Stealing]()算法，并行地处理任务，不保证处理顺序。
>
### 考点分析
